...
import {
  CartesianGrid,
  Dot,
  Line, // <-- pastikan ini dari recharts
  type LineProps,
  Pie,
  type PieProps,
  PieChart,
              ...
              return (
                <div key={item.dataKey || index} className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-2">
                    {indicator === "dot" && <span className="flex h-3 w-3 rounded-full" style={{ backgroundColor: item.color }} />}
                    {indicator === "line" && <span className="flex h-3 w-1 rounded-full" style={{ backgroundColor: item.color }} />}
                    {indicator === "dashed" && (
                      <span className="flex h-3 w-3 rounded-full border-2 border-dashed" style={{ borderColor: item.color }} />
                    )}
